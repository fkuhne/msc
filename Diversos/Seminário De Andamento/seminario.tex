...
A aproximação de $\dot{\bf x}$ através do método de Euler dá o seguinte sistema em tempo discreto:
...
\noindent onde $T$ é o período amostral e $k$ é o passo amostral. A equação (\ref{eqn:discretemodel}) pode ser escrita na seguinte forma compacta:
...
Um modelo linear pode ser obtido através de um modelo de erro com respeito a uma trajetória de referência. Para isso, considera-se um robô de referência descrito através do mesmo modelo~(\ref{eqn:discretemodelshort}). Portanto, sua trajetória ${\bf x}_r(k)$ e o controle ${\bf u}_r(k)$ são relacionados por
...
Expandindo o lado direito de ~(\ref{eqn:discretemodelshort}) através de séries de Taylor em torno de um ponto $({\bf x}_r(k),{\bf u}_r(k))$ e desconsiderando termos de ordem superior, tem-se que:
...
Logo, a subtração de ~(\ref{eqn:referencecar}) em~(\ref{eqn:taylor}) resulta em:
...
Como dito na Seção~\ref{sec:intro}, a essência de um esquema MPC é a otimização do comportamento do processo através das entradas de controle. Tal predição é feita através o modelo do processo dentro de um intervalo de tempo finito, usualmente chamado de {\em horizonte de predição}. Em cada tempo amostral, o controlador preditivo gera uma seqüência ótima de controle através da resolução de um problema de otimização. O mesmo problema é resolvido novamente para o proóximo instante amostral utilizando os estados atualizados e um horizonte de predição deslocado.

Para tornar a análise do algoritmo mais simples, considera-se neste trabalho que os estados estão sempre disponíveis para medição e que não existe diferença entre a planta e o modelo. Ainda, para a obtenção do controlador linear, considera-se que o par $({\bf A}(k),{\bf B}(k))$ é estabilizável.
...
\noindent onde $N$ é o horizonte de predição e ${\bf Q}$, ${\bf R}$ são matrizes de peso, com ${\bf Q}\geq 0$ e ${\bf R}>0$. A notação $a(m|n)$ indica o valor de $a$ no instante $m$ calculada no instante $n$.

Agora, o problema de otimização pode ser escrita como o problema de achar $\tilde{\bf
u}^\star$ tal que:
\begin{equation*}
...
O problem de minização da função de custo (\ref{eqn:cost}) é resolvido em cada passo $k$ resultando então, para cada $k$, uma sequeência de controle ótima $\{\tilde{\bf u}^\star(k|k),\cdots,\tilde{\bf u}^\star(k+N-1|k)\}$ e o custo ótimo $\Phi^\star(k)$. a lei de controle preditivo é implicitamente dada pelo primeiro termo da sequência de controle ótima, $\tilde{\bf u}^\star(k|k)$. Um diagrama de blocos com todos os componentes do sistema é mostrado na Fig. \ref{fig:bloco}, onde os índices $(k|k)$ foram omitidos.
...
A fim de formular o problema de otimização na forma padrão de programação quadrática, são definidos agora os seguintes vetores "empilhados":
...
Conseqüentemente, é possível, de ~(\ref{eqn:error}), escrever $\bar{\bf x}(k+1)$ como:
...
A matriz ${\bf H}$ é conhecida como a matriz {\em Hessiana} e, por definição, é positiva definida. ${\bf H}$ descreve a parte quadrática da função de custo, e o vetor ${\bf f}$ descreve a parte linear. ${\bf d}$ é independente de $\tilde{\bf u}$ e não influencia na determinação de $\bf u^\star$.
...
Como a variável livre de otimização é $\tilde{\bf u}(k)$, a restrição~(\ref{eqn:uconstr}) precisa ser reescrita com relação a esta variável. Assim:
...
Tabela~\ref{table:table1} mostra alguns resultados relacionando o custo computacional e o erro $\varepsilon$ como função do horizonte de predição $N$. O tempo de computação é o tempo médio para resolver o problema de otimização para um passo da trajetória. O número de flops necessários para completar os cálculos para um tempo amostral também é mostrado.
 
\begin{table}[htpb]
 \caption{Influência do horizonte de predição no tempo de computação e em $\varepsilon$}
 \label{table:table1}
 \centering
 \begin{tabular}{cccc}
  \hline
...
Obviamente, em uma aplicação em tempo-real, o horizonte de predição precisa ser escolhido de forma que o tempo de computação seja menor que o período amostral, $T$. Neste caso, $T=0,1$ segundos. Portanto, com $N=20$ o problema não é mais factível em tempo-real. Ainda. o esforço computacional com respeito ao número total de flops pode ser utilizado como medida de performance. Analizando o númerode flops e o valor de $\varepsilon$, foi escolhido $N=5$ como a melhor opção, considerando que o tempo computcional é cerca de dez vezes menor que o período amostral e o número de flops é cerca de seis vezes menor que o número de flops para $N=10$. Nota-se também que para $N\geq10$ o erro $\varepsilon$ não pode ser mais considerado como um fator decisivo. A simulação foi realizada em um computador com processador Athlon 2600+ rodando sistema operacinal Linux.
 

\section{Resultados experimentais}
\label{sec:exp}

Fig.~\ref{fig:twil} mostra o robô móvel Twil deselvolvido em nossos laboratórios e utilizado nos experimentos. O corpo do robô possui uma geometria cilíndrica com 1,35m de altura e 0,30m de raio, utilizando um sistema de acionamento diferencial. O software instalado nesta plataforma é baseado em sistema operacional Linux com modificações para a implementações de tarefas em tempo-real, chamado RTAI~\cite{Dozio:2003}.
