...
Assim, considerando os seguintes parâmetros de sintonia:
\begin{equation*}
	N = 5, \qquad
	{\bf Q} = \begin{bmatrix}	
		1 & 0 & 0 \\
	...
	{\bf R} = \begin{bmatrix}
		...
		0 & 0.1 \end{bmatrix}
\end{equation*}
e ${\bf x}_0=[-0,2~~3~~0]^T$ como configuração inicial do robô, obtém-se os resultados que são mostrados nas figuras a seguir.
...
A condição final dos estados para o caso apresentado acima é de ${\bf x}_f=[-0,83~~0~~\pi/2]^T$. Observa-se então claramente que o erro em regime antes existente na variável de posição $y$ transfere-se para $x$ quando da rotação do ponto de equilíbrio em $\pi/2$, dada a simetria existente entre estas duas variáveis.
...
Foi visto que, com as modificações apresentadas em \cite{essen01}, é possível diminuir consideravelmente o erro em regime existente em um dos estados de configuração. Entretanto, um erro de cerca de 0,003~m em $y$ parece ser persistente, pelo menos nos casos apresentados. 
...
Então, observa-se que, com a transformação em coordenadas polares na função de custo, sem aumentar o horizonte de predição e sem adição de outros termos, todos os estados convergem assintoticamente para a origem, sem apresentar erro em regime. Neste caso, o robô estabiliza exatamente na origem.
...
\end{alignat*}
com $\bf D$ e $\bf d$ definidos pela expressão~\req{eqn:mtx_restu}. Utilizando-se os seguintes parâmetros de sintonia:
\begin{equation*}
	N=5, \quad \lambda=2, \quad h=2, \quad {\bf R} = \begin{bmatrix}
		0.1 & 0 \\ 
		0 & 0.1 \end{bmatrix},
...
Na Figura~\ref{fig:comps_control_02} observa-se a diferença mais marcante: enquanto que a lei de controle gerada pelo NMPC (linha contínua) permanece dentro dos limites impostos pelas restrições, a lei de controle variante no tempo (linha tracejada) ultrapassa e muito estes limites. Na Figura~\ref{fig:comps_origin_02} é visto o detalhe da origem no plano $XY$. Nota-se a baixa taxa de convergência e o comportamento oscilatório gerado pela lei de controle variante no tempo.
\begin{figure}[H]\begin{center}
...
Esta lei de controle é definida na Seção~\ref{sec:sordalen}. A condição inicial do robô é de \mbox{${\bf x}_0=[0,5~~1~~-\pi/2]^T$} e os resultados são mostrados abaixo.
\begin{figure}[H]\begin{center}
...
Assim, observa-se nas figuras acima que o aumento do horizonte de predição, no caso apresentado, não é justificável, visto que não existe um significativo aumento de desempenho ao aumentar o horizonte de $N=5$ (linha contínua) para $N=12$ (linha tracejada).
