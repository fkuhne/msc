...
\begin{abstract}
This paper presents an optimal control scheme based on model-based predictive control (MPC) for a wheeled mobile robot (WMR) with nonholonomic constraints. In comparison to classical approaches to the control of WMRs, it is shown that, by using MPC, some advantages can be obtained, such as the ability to handle constraints due to state or input limitations and performance improvement. A cost function in polar coordinates is proposed and the point stabilization problem is solved. Considerations regarding the computational effort of the MPC are developed with the purpose of analysing the viability of the proposed technique in real-time.
\end{abstract}
\begin{keywords}
...
%	\dot x	  = v\cos\theta \quad\qquad
%	\dot y	  = v\sin\theta \quad\qquad
%	\dot \theta = w
%\end{equation}
\begin{equation}\label{eqn:model}
	\left\{
...
\end{multline}
which will be called as the original cost function in cartesian coordinates, where $N$ is the prediction horizon and ${\bf Q}\geq 0$, ${\bf R}>0$ are weighting matrices used to penalize the state error and the control effort, respectively.
...
\end{alignat}
where ${\bf x}_0$ in~\req{eqn:restci} is the initial condition which corresponds to the value of the states measured at the actual instant. Constraint~\req{eqn:restmodel} represents the prediction model and~\req{eqn:restx1}-\req{eqn:restu1} are a general form to represent linear bounds in the state and control variables, and they may or may not be present in the optimization problem. Note that we can rewrite~\req{eqn:restx1}-\req{eqn:restu1} in the form of~\req{eqn:restx}-\req{eqn:restu} with:
...
In this section we focus on the formulation of MPC for the WMR considering a cost function in cartesian coordinates. In order to evaluate the performance of the approach, simulation results are shown\footnote{All the optimization problems of this paper has been solved with the {\sc Matlab} routine {\tt fmincon}.}.
...
Initially, the cost function to be minimized is that showed in~\req{eqn:cost}. Then, a second example is shown with some modifications to the cost function, proposed in~\cite{essen01}. In both cases, constraints only in the amplitude of the control inputs based on the robot Twil, Eq.~\req{eqn:const} will be taken into account.

\subsection{Original Cost Function in Cartesian Coordinates.}\label{sec:pure}
The weighting matrices used here are ${\bf Q}={\rm diag}(1,1,0.5)$ (equal penalty in the position states and a half of the penalty in the orientation) and ${\bf R}={\rm diag}(0.1,0.1)$ (less penalty in the control effort). The prediction horizon is $N=5$. The initial configuration is \mbox{${\bf x}_0=[0~~6~~0]^T$}, and the goal is the origin. The obtained simulation results are showed in Figures~\ref{fig:traj_01}-\ref{fig:control_01}.
\begin{figure}[htbp]
	\centering
...
It can be seen in Fig.~\ref{fig:traj_02} that the large steady-state offset in $y$-state has disappeared. Also, notice in Fig.~\ref{fig:control_02} that the convergence rate, in comparison to the latter example, is higher. In the example of Section~\ref{sec:pure} the stabilization time is about 40 seconds, while now this is 21 seconds. The control signals respect the imposed constraints.

However, the coupling between the states $x$ and $y$ remains. Even if it can be considered small, the final $y$-state still presents a persistent offset. The final configuration is ${\bf x}_f=[0~~0.006~~0]^T$. 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{MPC with Cost Function in Polar Coordinates}\label{sec:polar}

A possible way to overcome the state offset, would be the use of some coordinate transformation able to decouple the position states $x$ and $y$ from the same control input, $v$. This idea is developed here, by using the following state transformations in polar coordinates, as proposed in~\cite{lages98b}:
\begin{equation*}
...
\subsection{NMPC with Constraints in Control Variables}
Changing the cost function in~\req{eqn:optim} by \req{eqn:polar_cost} and considering the same tunning parameters and initial condition used in the examples of Section~\ref{sec:cartesian}, the following results showed in Figures~\ref{fig:traj_03}-\ref{fig:control_03} are obtained. Once more, constraints in the amplitude of the control inputs described by~\req{eqn:const} are taken into account.
\begin{figure}[htbp]
	\centering
...
Then, comparing the results above with the results obtained in Section~\ref{sec:cartesian}, significative performance improvements with respect to both state and control trajectories can be noted. It can be seen in Fig.~\ref{fig:traj_03} that the $x$-state presents a maximum amplitude of about $0.3~m$, while for the example in Section~\ref{sec:essen} the $x$-state needs almost $3~m$ to maneuver towards the origin (Fig.~\ref{fig:traj_02}). Furthermore, in Fig.~\ref{fig:control_03} the control inputs are smoother than the ones presented in Fig.~\ref{fig:control_02}.

Note also that, with the transformation in polar coordinates, the decoupling between the position states $x$ and $y$ in the cost function has been possible. Now the final configuration is ${\bf x}_f=[0~~0~~0]^T$. Furthermore, it has been possible to improve the overall performance without the inclusion of another terms in the cost function or a longer prediction horizon. 
...
Fig.~\ref{fig:traj_04} shows the trajectory of the robot in the XY-plane, where the gray polygons stands for the walls of the corridor. It is straightforward to note that the configuration space is nonconvex which, for an optimization problem such the one solved here, is unacceptable~\cite{mayne00}. However, this problem can be overcome by splitting the configurations space in two convex halfspaces, thus solving a different optimization problem for each one of these halfspace. In the case presented here, $x=-1$ defines the limit between the two halfspaces. Then, we include state constraints as defined by~\req{eqn:restx} and for each halfspace we have:
\begin{equation*}
	\left\{
		\begin{aligned}
			{\bf x}_r &= [0~4~0]^T,~~{\bf c}=[-1~5~\infty~\infty~-4~\infty]^T, &\text{if}~x<-1 \\
			{\bf x}_r &= [0~0~0]^T,~~{\bf c}=[1~5~\infty~1~\infty~\infty]^T, &\text{if}~x\geq -1
		\end{aligned}
	\right.
\end{equation*}
where ${\bf x}_r$ is the reference point in which the robot must converge, $\bf C$ and $\bf c$ are defined in~\req{eqn:const2} and $\infty$ stands for a unconstrained variable. Further details of this approach can be seen in~\cite{kuhne05}.

\subsection{Comparison with Classical Approaches.}\label{sec:comp}
As pointed out in Section~\ref{sec:intro}, classical approaches to the control of nonholonomic WMRs include non-smooth and time-varying control laws. In this section a comparative analysis between the proposed NMPC and some of these approaches are therefore carried out. The goal is once again the origin. For instance, let us consider the time-varying control of~\cite{samson91} and the non-smooth control of~\cite{canudas92}. Thus, with a initial condition of ${\bf x}_0=[-1~~3~~0]^T$ and considering control constraints~\req{eqn:const} in the NMPC, we have the results in Figures~\ref{fig:traj_05}-\ref{fig:control_05}, where the continuous line stands for the proposed NMPC, the dashed line stands for the time-varying control law and the dash-dotted line stands for the non-smooth control law.
...
It is straightforward to note the typical features of the time-varying control law: low convergence rate and oscillatory movements. With the use of the non-smooth control, these problems does not happen anymore. 
However, the performance achieved with NMPC is even better. Furthermore, note that the classical control laws violate the amplitude control constraints, while with the NMPC they are respected.
...
The data in Table~\ref{tab:comp} provides enough evidence that a standard of-the-shelf computer is able to run a MPC-based controller for a WMR. Note that for $N=5$ all of the examples are feasible in real-time. However, results presented in Section~\ref{sec:pure} show the existence of steady-state offset and the results in Section~\ref{sec:essen} show poor performance. Thus, among all of them, the approach that performs the best results with respect to state and control trajectories and computational effort is the one proposed in Section~\ref{sec:polar}, the NMCP with cost function in polar coordinates.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Conclusion}\label{sec:conclusions}

This paper has presented an application of model predictive control to the problem of point stabilization of a nonholonomic wheeled mobile robot. A steady-state offset in one of the position variable has been identified and a transformation of the configuration variables into polar coordinates has been successfully proposed to solve the problem. Through some examples, it was shown an important advantage of MPC: to handle constraints in a straightforward way. The obtained control signals were such that the constraints imposed on the control variables were respected. 
