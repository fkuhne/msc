...
% as opções adequadas para o formato do CBA, ao contrário do arquivo
...
\begin{itemize}
  ...
  Brasileiro de Automática} na produção das versões acabadas dos artigos
  nos anais.
\end{itemize}
...
\end{verbatim}
é utilizado no início deste arquivo para geração do formato final para
o \emph{Congresso Brasileiro de Automática}.
...
Em geral, para cada comando \verb+\autor+ deve haver um comando
\verb+\address+ correspondente, salvo o caso de compartilhamento de
endereços. Neste caso, omite-se o comando \verb+\address+ para o
segundo autor e indica-se o número do endereço que se deseja
...
\textbf{IMPORTANTE:} Caso esteja se produzindo um texto com a opção
\verb+conference+, colocam-se os comandos que vão de \verb+\maketitle+ a
\verb+\keywords+ dentro de um ambiente \verb+twocolumn+. Isto é
necessário para que os resumos sejam formatados em coluna
simples\footnote{Como este arquivo está adaptado para utilizar o formato 
adequado para o \emph{Congresso Brasileiro de Automática}, isto já está feito}.
Veja seção~\ref{sec:esqueleto}.
...
referências~\cite{latex,latex:guide,latex:companion}. Note-se que não
é necassário definir o estilo bibliográfico com o comando
\verb+\bibliographystyle+, pois o mesmo é definido automaticamente
pelo estilo \SBATeX. Basta incluir o arquivo de bibliografias, neste
exemplo o artigo \verb+exemplo.bib+ por meio do comando.
\begin{verbatim}
    ...
  ...
  \end{document}
\end{verbatim}
Note-se que este comando deve ser colocado logo antes do encerramento
do artigo, o que é feito pelo comando \verb+\end{document}+, de tal
forma a guarantir que a bibliografia seja o último item do artigo.


\subsection{Esqueleto deste arquivo no formato \SBATeX}
\label{sec:esqueleto}

Este arquivo, adaptado para gerar o formato adequado para o \emph{Congresso
Brasileiro de Automática}, faz uso da seguintes intruções:
\begin{verbatim}
  \documentclass[conference,harvard,brazil,english]{sbatex} 
  \usepackage[latin1]{inputenc}
  \usepackage{ae}
