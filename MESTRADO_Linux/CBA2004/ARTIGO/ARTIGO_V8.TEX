...
%Discretizando o modelo \req{eqn:x_cont} pelo método de Euler ($\dot {\bf x}\approx\frac{{\bf x}(k+1)-{\bf x}(k)}{T}$), tem-se:
...
\section{Controle por realimentação linearizante}\label{sec:control}
	...
	Conforme \citeasnoun{isidori1}, faz-se agora a dedução da realimentação linearizante para a posição robô, denotada por $[x_c~~y_c]^T$, relacionado ao centro de massa do robô. Para existir uma realimentação linearizante, é necessário o deslocamento de $[x_c~~y_c]^T$ para um outro ponto qualquer no corpo do robô \cite{yamamoto1}. Assim, elege-se um ponto, {\em a priori} arbitrário, da estrutura do robô como ponto de referência, dado pelo sistema de coordenadas $\{R,X_r,Y_r\}$ (vide Figura \ref{fig:robo}). A posição deste ponto com relação ao sistema global é dada por:
...
\end{equation}
	...
	Pode ainda ser utilizado aqui o {\em Princípio da Equivalência à Certeza Presumida} \cite{goodwin1}, onde presume-se que uma lei de controle para um sistema com incertezas pode ser deduzida desconsiderando estas incertezas. Assim, considerando o modelo \req{eqn:x_cont}, e a expressão de saída como sendo a posição do ponto $R$, tem-se:
\begin{align*}
...
\end{align*}
	...
	O método de linearização entrada-saída consiste em diferenciar a expressão de saída até que a entrada ${\bf u}({\bf x})$ apareça na expressão. Percebe-se que isto acontece logo na primeira diferenciação. Assim, define-se a seguinte matriz:
...
\end{equation}
	...
	Fica evidente aqui a necessidade de definir um ponto $R$ que não esteja sobre o eixo comum às rodas fixas. Pontos sobre este eixo (como o ponto $C$) podem mover-se instantaneamente em apenas uma direção, enquanto que os demais pontos do corpo do robô podem mover-se em duas direções \cite{lages1}. Vê-se, da equação \req{eqn:beta}, que para a inversa de $\beta(q)$ existir, $x^c_r$ precisa ser diferente de zero. 

	Assim, a expressão \req{eqn:u} é o controle que lineariza o modelo do sistema \req{eqn:diff_y},
...
\end{equation*}
onde ${\bf L}$ é uma matriz de ganhos positivos e ${\bf e}_s$ é o erro entre a posição de referência do ponto $R$, $\overline{\bf y}_s$, e a posição atual do ponto $R$, ${\bf y}_s$. Assim:
\begin{equation*}
	{\bf L} =
		\begin{bmatrix}
			l_x & 0 \\
...
\section{Estimação dos estados por filtro de Kalman}\label{sec:ekf}
	...
	Assim, em um sistema onde os estados são não observáveis, torna-se necessário o uso de sensores capazes de fazer a leitura destes estados. Como o uso de sensores envolve a modelagem dos mesmos, pode-se considerar ruídos relacionados também à observação do sistema. 
...
\begin{align*}
	f({\bf x}(k)) &=
		...
		\end{bmatrix},
\end{align*}
onde $T$ é o período de amostragem e $k$ é o instante amostral.	${\bf w}(k)$ e ${\bf v}(k)$ são vetores de ruído branco (com distribuição normal e média zero) de processo e de medida, respectivamente, dados pelas seguintes expressões de covariância: 
\begin{align}
...
\end{align}
\noindent$E\{\cdot\}$ é o operador valor esperado, ${\bf Q}(k)$ e ${\bf R}(k)$ são matrizes diagonais formadas pelas variâncias de ${\bf w}(k)$ e ${\bf u}(k)$, respectivamente. A equação \req{eqn:wv} indica que os ruídos de processo e de medida são descorrelacionados. 
...
\end{equation}
onde ${\bf I}$ é uma matriz identidade e ${\bf A}(k)$, ${\bf B}(k)$ e ${\bf C}(k)$ são matrizes que descrevem o modelo linear ${\bf x}(k+1)= {\bf A}(k){\bf x}(k)+{\bf B}(k){\bf u}(k)$ e ${\bf y}(k)={\bf C}(k){\bf x}(k)$.
...
\end{equation}
	...
	Na Figura \ref{fig:bloco1}, é mostrado o sistema em malha fechada, agora considerando o filtro de Kalman para a estimação dos estados do modelo.
\begin{figure}[H]\begin{center}
