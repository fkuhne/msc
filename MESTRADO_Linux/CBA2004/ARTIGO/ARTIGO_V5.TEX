...
\begin{equation}\label{eqn:x_cont}
	...
	\end{bmatrix},
\end{equation}
onde $u_1$ é a velocidade tangencial e $u_2$ é a velocidade angular. Considerando a discretização do sistema pelo método de Euler $(\dot {\bf x}\approx\frac{{\bf x}(k+1)-{\bf x}(k)}{T})$ e uma expressão de observação, tem-se que:
\begin{align}
...
\end{multline*}
onde ${\bf F}(k)$ e ${\bf H}(k)$ são os jacobianos de $f({\bf x}(k))$ e $h({\bf x}(k))$, respectivamente, avaliados em $\hat {\bf x}(k|k)$ \cite{mutambara1}. 
...
\section{Controle da posição por realimentação linearizante}\label{sec:control}
	...
	Pode ainda ser considerado aqui o {\em Princípio da Equivalência à Certeza Presumida} \cite{goodwin1}, onde presume-se que uma lei de controle para um sistema com incertezas pode ser deduzida desconsiderando estas incertezas. Assim, reescrevendo o sistema \req{eqn:x_cont} em tempo contínuo e livre de ruídos de processo, e considerando a expressão de observação como sendo a posição do ponto $R$, ${\bf y}^p_R(x)=[x_r~~y_r]^T$, sem ruídos de medida, tem-se:
\begin{align}
...
\end{center}\end{figure}
	...
	É fácil de verificar a estabilidade do sistema em malha fechada. Reescrevendo a expressão \req{eqn:diff_y} como $\dot {\bf y}^p_R=L{\bf y}^p_{ref_R}-L{\bf y}^p_R$, nota-se que para o sistema ser estável, ${\bf L}$ precisa ser positiva definida.
