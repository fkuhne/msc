...
\end{align*}
	...
	O método de linearização entrada-saída consiste em diferenciar a expressão de saída até que a entrada ${\bf u}({\bf x})$ apareça na expressão \cite{isidori1}. Percebe-se que isto acontece logo na primeira diferenciação. Assim, tem-se o seguinte modelo:
\begin{equation}\label{eqn:diff_y}
	\dot {\bf y}_s({\bf x}) = \alpha({\bf x}) + \beta({\bf x}){\bf u}({\bf x}),
\end{equation}
que é linearizado aplicando-se o seguinte controle linearizante:
\begin{equation}\label{eqn:u}
	{\bf u}({\bf x}) = \beta({\bf x})^{-1}{\bf v}({\bf x}) - \alpha({\bf x}),
\end{equation}
onde $\alpha({\bf x})=0$, pois $f({\bf x})=0$, e
...
\end{split}\end{equation}
	Fica evidente aqui a necessidade de definir um ponto $R$ que não esteja sobre o eixo comum às rodas fixas. Pontos sobre este eixo (como o ponto $C$) podem mover-se instantaneamente em apenas uma direção, enquanto que os demais pontos do corpo do robô podem mover-se em duas direções \cite{lages1}. Assim, da equação \req{eqn:beta}, para a inversa de $\beta({\bf x})$ existir, $x^c_r$ precisa ser diferente de zero. 
...
\end{align}
\noindent$E\{\cdot\}$ é o operador valor esperado e ${\bf Q}(k)$ e ${\bf R}(k)$ são matrizes diagonais formadas pelas variâncias de ${\bf w}(k)$ e ${\bf v}(k)$, respectivamente. A expressão \req{eqn:wv} indica que os ruídos de processo e de medida são descorrelacionados. 
...
\end{equation}
onde ${\bf K}(k)$ é o ganho do filtro de Kalman, ${\bf P}(k|k)$ é a covariância do erro de estimação e ${\bf A}(k)$, ${\bf B}(k)$ e ${\bf C}(k)$ são matrizes que descrevem o modelo linear $\mbox{{\bf x}(k+1)= {\bf A}(k){\bf x}(k)}+{\bf B}(k){\bf u}(k)$, ${\bf y}(k)={\bf C}(k){\bf x}(k)$.
...
\begin{align}
	...
	{\bf y}_{bus}(k) &=
		\begin{bmatrix}
			\theta_{bus}(k)
		\end{bmatrix} + {\bf v}_{bus}(k)\label{eqn:y_bus}
\end{align}
...
\end{center}\end{figure}
	...
	{\bf 1.} Obedecendo ao critério de minimização do erro médio quadrático, calcula-se o ganho do filtro de Kalman \req{eqn:K_nlin};
...
\end{equation*}
	...
	Neste ponto, as grandezas ${\bf e}(k)$ e ${\bf E}(k)$ de cada nodo são tansmitidos para os outros nodos através de {\em sockets}. Faz-se agora a fusão destes dados, modificando-se as equações \req{eqn:Pkk_nlin} e \req{eqn:xkk_nlin} para o caso descentralizado, conforme os passos a seguir:
...
\begin{figure}[H]\begin{center}
    \includegraphics[width=.97\linewidth]{Figuras/oito20.eps}
    \caption{Efeito do aumento do ruído de processo.}
    \label{fig:oito20}
\end{center}\end{figure}
\begin{figure}\begin{center}
    \includegraphics[width=.97\linewidth]{Figuras/quad20.eps}
    \caption{Efeito do aumento do ruído de processo.}
    \label{fig:quad20}
\end{center}\end{figure}
