...
%
% O problema dessa versao eh q soh funciona para N=3. 
% Para N variavel, use kmpc8.
%
...
for k = 1 : Kf
    ...
    Aaux = mtx(thetaref(k),vref(k),T);
    Abar = Aaux;
    for i = 1 : N-1
        if (k+i > Kf)
            Aaux = Aaux*mtx(thetaref(Kf),vref(Kf),T);
        else
            Aaux = Aaux*mtx(thetaref(k+i),vref(k+i),T);
        end
        Abar = [Abar ; Aaux];
    end
    
    Ak1 = mtx(thetaref(k+1),vref(k+1),T);
    Ak2 = mtx(thetaref(k+2),vref(k+2),T);
    Bk0 = mtx(thetaref(k),T);
    Bk1 = mtx(thetaref(k+1),T);
    Bk2 = mtx(thetaref(k+2),T);    
    Zero = zeros(n,m);
    
    S = [Bk0         Zero    Zero;
         Ak1*Bk0     Bk1     Zero;
         Ak2*Ak1*Bk0 Ak2*Bk1 Bk2];
...
disp('em x:');
x(size(x,2))-xref(size(xref,2)-3)
disp('em y:');
y(size(y,2))-yref(size(yref,2)-3)
disp('em theta:');
theta(size(theta,2))-thetaref(size(thetaref,2)-3)
