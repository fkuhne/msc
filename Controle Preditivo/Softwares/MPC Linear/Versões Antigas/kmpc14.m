...
Lampl = [eye(m) ; -eye(m)]; L = Lampl;
rampl = [vmax ; wmax ; -vmin ; -wmin]; 
r = rampl;
for i = 1 : N-1;
    L = blkdiag(L,Lampl);
    r = [r ; rampl];
...
for k = 1 : Kf
    ...
    for i = 1 : N-1
        if (k+i > Kf) 
            thetar(Kf) = a*thetar(Kf-1)+(1-a)*thetaref(Kf);
            xr(Kf+1) = a*xr(Kf)+(1-a)*xref(Kf-1);
            vr(Kf) = (xr(Kf+1)-xr(Kf))/(cos(thetar(Kf)*T));
            A = A*mtx(thetar(Kf),vr(Kf),T);
        else 
            thetar(k+i) = a*thetar(k+i-1)+(1-a)*thetaref(k+i);
            xr(k+i) = a*xr(k+i-1)+(1-a)*xref(k+i);
            vr(k+i) = (xr(k+i)-x(k))/(cos(thetar(k+i)*T));
        ...
        for lin = 1 : N % loop das linhas
            if (lin > col) % se esta na triangular inferior
                ...
                if (k+col > Kf) 
                    ...
                    if (k+col-1<2) thetar(k+col-1) = theta(k);
                    else thetar(k+col-1) = a*thetar(k+col-1-1)+(1-a)*thetaref(k+col-1);
                    end
    ...
    for i = 1 : N-1
        if (k+i > Kf) urefbar = [urefbar ; vref(Kf) ; wref(Kf)];
        else urefbar = [urefbar ; vref(k+i) ; wref(k+i)];        
        end
    end
    ...
    eu = quadprog(H,f,L,r,Leq,req,lb,ub,x0,options);
