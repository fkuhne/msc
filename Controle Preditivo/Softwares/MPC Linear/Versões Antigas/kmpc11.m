...
t = 100; % quantidade de entre-periodos (para a simulacao); t > 1.
...
%     lambda = eig(A);
%     if (max(lambda)>0) disp('sistema instavel'); end
    Abar = A;
    for i = 1 : N-1
        if (k+i > Kf)
    ...
    % inter-sampling time of T/t:
%     for i = 0 : t-1
%         x(k+i+1) = x(k+i) + v(k)*cos(theta(k+i))*(T/t);   
%         y(k+i+1) = y(k+i) + v(k)*sin(theta(k+i))*(T/t);
%         theta(k+i+1) = theta(k+i) + w(k)*(T/t);
%     end
%     
%     x(k+1) = x(k+t);
%     y(k+1) = y(k+t);
