...
% simplified version (without rate constraints).
...
for k = 1 : kf
    ...
    for i = 1 : is
        ...
        x(ks+1) = x(ks) + v(k)*cos(theta(ks))*Ts + 1e-3*randn(1);
        y(ks+1) = y(ks) + v(k)*sin(theta(ks))*Ts + 1e-3*randn(1);
        theta(ks+1) = theta(ks) + w(k)*Ts + 1e-5*randn(1);        
    end
