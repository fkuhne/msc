...
var s_dynamicAccountList="mathdev=www-external-dev.;mathIntranet=www-internal.;mathExtra=distribs/;mathGlobal,mathUK=.co.uk;mathGlobal,mathCH=.ch;mathGlobal,mathES=.es;mathGlobal,mathNordic=se.;mathGlobal,mathKR=.co.kr;mathGlobal,mathIT=.it;mathGlobal,mathDE=.de;mathGlobal,mathFR=.fr;mathGlobal,mathNL=.nl;mathGlobal,mathTools=mathtools.net;mathGlobal,mathmain=mathworks.com"
...
var s_linkInternalFilters="javascript:," + location.hostname
var s_linkLeaveQueryString=false
var s_linkTrackVars="None"
var s_linkTrackEvents="None"

var s_cookieDomainPeriods="2"
var s_cookieDomainList="mathworks.co.kr,mathworks.co.uk";
var d=s_split(s_cookieDomainList,","),h=location.hostname;
for (var i=0; i<d.length; i++){if(h.indexOf(d[i])>=0)s_cookieDomainPeriods="3";}
 ...
 */
function s_split(s,p)
	{var i=0,a=new Array(),j;if(s&&p){while((s.length>0)&&(p.length
	>0)){j=s.indexOf(p);if((!j)&&(p!=s.substring(0,p.length)))break;
	if(j==-1){a[i++] = s;break;}a[i++]=s.substring(0,j);s=s.substring
	(j+p.length,s.length)}}return a}
/*
